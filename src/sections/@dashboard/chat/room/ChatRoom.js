...
export default function ChatRoom({ conversation, participants }) {
  ...
  const renderContent = (
    <>
      {isGroup ? (
        ...
        <ChatRoomSingle participant={participants[0]} isCollapse={openInfo} onCollapse={() => setOpenInfo(!openInfo)} />
      )}
  ...
  return (
    <Box sx={{ position: 'relative' }}>
      ...
      >
        <Iconify width={16} icon={openNav ? 'eva:arrow-ios-forward-fill' : 'eva:arrow-ios-back-fill'} />
      </StyledToggleButton>
